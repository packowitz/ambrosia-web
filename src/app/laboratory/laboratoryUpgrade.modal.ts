...
@Component({
    selector: 'laboratory-upgrade-modal',
    template: `
        <div class="ma-2">
          ...
          <div class="mt-2 flex-center">
            Upgraded Laboratory provides more incubators and speeds up cloning
          </div>
          <ion-item class="mt-2">
          ...
          <div *ngIf="!getBuilding().upgradeTriggered">
            <div class="mt-2 flex-center" *ngIf="!upgradeSeconds">Cannot upgrade Laboratory higher than level {{getBuilding().level}}</div>
            <div class="mt-3" *ngIf="upgradeSeconds">
              ...
              <div class="mt-2 flex-center">
                ...
                <ion-button [disabled]="!hasEnoughResources || saving || model.upgrades.length >= model.progress.builderQueueLength" (click)="performUpgrade()">Upgrade Laboratory to level {{getBuilding().level + 1}}</ion-button>
              </div>
            </div>
          </div>
...
export class LaboratoryUpgradeModal {
