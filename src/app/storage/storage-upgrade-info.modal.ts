...
@Component({
    selector: 'storage-upgrade-info-modal',
    template: `
      <div class="ma-2 popover-scrollable" *ngIf="building">
        <div class="flex-space-between">
          <strong>The Storage</strong>
          <ion-button size="small" fill="clear" color="dark" (click)="closeModal()">
            <ion-icon name="close"></ion-icon>
          </ion-button>
        </div>
        <div class="mt-2">Upgrade Storage to gain more space for your resources.</div>
          ...
          <div class="flex-space-between">
            ...
            <div class="flex-grow flex-end">
              <div *ngFor="let cost of buildingService.getUpgradeCosts(buildingType, level)" class="flex ml-1">
                {{cost.value1}}<img src="assets/icon/resources/{{cost.resourceType}}.png" class="resource-icon">
              </div>
            </div>
          </div>
...
export class StorageUpgradeInfoModal {
    ...
    getUpgrades(level: number): string[] {
        return this.propertyService.getProps(this.buildingType + '_BUILDING', level)
            .map(p => '+' + p.value1 + ' ' + this.converter.readableIdentifier(p.resourceType));
    }
}